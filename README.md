...
- items - список товаров, если точнее, то это список идентификаторов твоаров.
...
- baseUrl - базовый УРЛ, который будет подставляться в fetch.
- options - поле опции, в него прописываются настройки, которые добывляюся в headers, в fetch.  

Конструктор класса принимает на вход базовый УРЛ и настройки в формате объекта.
Урл прописывыется в поле, настройки добавляются в поле, по умолчанию там уже прописывается объект такого типа  
 headers:  
  {  
...
Методы класса:
- Метод get принимает на вход строковое добавление, которое присовокупляется к базовому УРЛ. Метод возвращается запрос fetch.
...
Конструктор принимает аргументом базовый УРЛ и CDN, внутри происходит вызов родительского конструктора и запись единственного поля.
...
- _image - здесь хранится ДОМэлемент, в котором отображается картинка товара.
- _price - здесь хранится ДОМэлемент, в котором отображается цена товара.
- _description - здесь хранится ДОМэлемент, в котором отображается описание товара.
- _category - здесь хранится ДОМэлемент, в котором отображается категория товара.
- _button - здесь хранится ДОМэлемент, в котором отображается кнопка "Добавить/убрать" товара.  
...
#### Класс Order.  
Отвечает за отображение начала оформления заказа.
 Поля класса:
...
#### Класс Contacts.
Класс отвечает за отображение окна заполнения контактов.  
...
### Презентер.
Презентер расположен в файле index.ts.

В начале создаются экземпляры классов, необходимые для работы проекта. Затем перечислены события. В конце инициируется запуск метода getProducts, который получает массив карточек продукта. В ветке then этот массив обрабатывает метод модели setItems, сохраняя его в поле items, этот метод также инициирует событие 'items:change'. Далее каждое событие будет разобрано отдельно.


#### События из брокера событий.

- 'items:change' - колл-бэк события принимает аргументом массив с данным карточек, он вызывает сеттер объекта page, присваивая ему массив отрендеренных карточек товара, для каждой карточки создается объект класса Card и вызывается метотд рендеринга.

- 'card:select' - событие выбранная карточка возникает, когда пользователь кликнул ну одну из карточек. В этот момент коллбэк инициирует в слушателе событие 'card:select', аргументу ему предается объекта типа IProduct. Функция внутри этого события запускает сеттер в модели, прописывая объект в поле preview. Этот же метод объявляет о начале события preview:change которому передается тот же самый объект типа IProduct, но он уже берется из поля.

- 'preview:change' - событие рендерит карточку из шаблона расширенной карточки. Здесь также прописывается колл-бэк для слушателя кнопки "добавить в корзину". Далее вызыввется метод рендеринга модального окна, с аргументом отрендеренной карточки. Колл-бэком для создания объекта класса Card передается функция обработчик слушателя "добавить в корзину", при нажатии на кнопку вызыввается метод модели addToBasket(добавление в корзину), если товар уже есть в корзине, то метод удаления из корзины. Оба эти метода изменяют поле корзины в модели и инициируют событие 'basket:change' изменение корзины.

- 'modal:open', когда модальное окно открывается, генерируется событие 'modal:open'. Оно запускает метод объекта page, который фиксирует страницу, не давая ей скролиться.

- 'modal:close' при закрытие окна, запускается данное событие, и скроллинг вновь разрешается.

- 'basket:change' - колл-бэк обработчик события не принимает аргументов, он берет данные из модели, создает объект класса Card с шаблоном карточки для корзины и преобразует корзину, то есть ее внутреннюю верстку исходя из данных модели о корзине. В списке появляется выбранный товар, меняется итоговая сумма. Также меняется цифра в счетчике кнопки корзины на главной странице, это происходит через метод-сеттер объекта page.

- 'basket:open' - при нажатии на кнопку корзины, которая находится в шапке главной страницы инициируется данное событие. Сдушатель на эту кнопку устанавливается в конструторе объекта Page. Запускается метод рендеринга модального окна с аргументом, элементом(объектом, где ключ элемент), который рендерится методом внутри объекта basket. Корзина со списком выбранных товаров открывается. Внутри нее у каждой карточки есть кнопка удаления товара, при ее нажатии инициируется метод модели удаление из корзины с аргументом id товара. Из- за чего снова происходит событие 'basket:change' и корзины перерисовывается. Если пользователь решит начать процесс оформления заказа, он нажемет на соответмтвующую кнопку. Слушатель которой инициирует следующее событие.

- 'order:open' - здесь методу рендеринга модального окна отдается результат рендеринга соответствующего метода объекта orderForm. Внутри формы есть два инпута, слушатели на них устанавливается в конструкторе объекта. Если происходит изменение в поле ввода, запускается метод родительского класса Form, который генерирует событие изменения поля, передавая объект данных, название поля, значение в поле.

- /^order\..*:change/ - событие изменения в инпуте в форме заказа. Принимая данные функция события передает данные модели, которая сохраняет изменения в соответсвующих полях поля order внутри модели. Здесь же автоматически инициипуется валидация поля order, после чего генерируется следующее событие. 

- 'Errors:change' - изменение массива ошибок формы сбора данных о пользователе. Аргументом для подписчиков этого события является поле errorsOfForms, которое сохраняет в себе объект с ошибками валидации. 'Errors:change' принимая объект с ошибками валидации функция-подписчик на событие анализирует эти данные, если два поля из первой формы Ордер заполнены, кнопка "Далее" разблокируется. При нажатии на нее происходит следующее событие. 

- 'order:submit' - в модальном окне рендерится следующее окно "Контакты". Здесь же я запускаю валидацию формы, это нужно для того, чтобы я мог заполнить данные, закрыть окно, снова его открыть, и поля отвалидировались со значениями полей из памяти сессии. В окне сбора контактных данных я также слушаю два инпута, сохраняю в модель и валидирую, как и с предыдущим окном. Здесь запускается событие /^contacts\..*:change/. Принцип его работы точно такой, же как событием order. Если все поля прошли заполнены и прошли валидацию, разблокируется кнопка "оплатить". При нажатии на нее инициируется следующее событие.

- 'contacts:submit' событие оплаты заказа. Функция запускает метод АПИ sendOrder, передавая ему аргументом данные о заказе из модели. Если сервер дает ответ о положительном исходе, в модальном окне рендерится окно об успешной оплате, создается экземпляр объекта Success, внутри которого вешается слушатель на кнопку "К новым покупкам", здесь мы аргументом передаем колл-бэк для этого слушателя. Для рендеринга этого окна, нам нужно значени общей суммы заказа, ее мы передаем в функцию рендеринга как результат ответа сервера. ТАк как заказ прошел успешно, данные о товарах в корзину внутри модели обнуляются. Счетчик кнопки корзины также обнуляется. В модели это делает метод emptyBasket, который сообщает о событии "Корзина изменилась". При закрытии модального окна, перед нами предстает главная страница с теми же самыми товарами.
