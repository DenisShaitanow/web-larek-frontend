...
export class Larek implements ILarek {
    ...
    constructor(larekModel: ILarekModel, emitter: IEvents, view: IView) {
        ...
        this.emitter.on('orderContinue', ((data: { pay: "personally" | "online", address: string; }) => this.orderContinue(data)));
    ...
    async renderProductList(): Promise<IProduct[]> {
        ...
        console.log(products['0'].imageSvg);
    ...
    async selectProduct(data: { id: string }): Promise<IProduct> {
        ...
        const settings: ProductSettings = {
            ...
            imageSvg: rawProduct.imageSvg
        };
        this.view.uppendElement(this.view.renderCard(settings, 'CardPreview', settings.image), 'modalContent', true);
        this.view.openModalWindow();
        return rawProduct;
    }
